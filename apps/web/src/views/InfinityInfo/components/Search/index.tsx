...
const Search = () => {
  ...
  const handleOutsideClick = (e) => {
    const menuClick = menuRef.current?.contains(e.target as Node)
    const inputCLick = inputRef.current?.contains(e.target as Node)
    const showMoreTokenClick = showMoreTokenRef.current?.contains(e.target as Node)
    const showMorePoolClick = showMorePoolRef.current?.contains(e.target as Node)
          ...
          <Menu ref={menuRef}>
            ...
            {tokensForList.slice(0, tokensShown).map((token) => {
              return (
                <HoverRowLink
                  onClick={() =>
                    handleItemClick(`/${infinityInfoPath}${chainPath}/tokens/${token.address}${stableSwapQuery}`)
                  }
            ...
            {poolForList?.slice(0, poolsShown).map((p) => {
              return (
                <HoverRowLink
                  onClick={() =>
                    handleItemClick(`/liquidity/pool/${chainIdToExplorerInfoChainName[chainId]}/${p.address}`)
                  }
